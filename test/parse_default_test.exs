defmodule DateFormatTest.ParseDefault do
  ...
  test "exceptions" do
    ...
    assert %NaiveDateTime{:year => 2013, :month => 3, :day => 2} =
  ...
  test "parse simple format YYYY-M-D, with padding variations" do
    ...
    assert {:error, "Expected `2 digit month` at line 1, column 6."} =
             parse("2013- 1- 8", "{YYYY}-{0M}-{0D}")
  ...
  test "parse ISO week" do
    ...
    # monday
    assert ~N[2018-12-31 00:00:00] = Timex.parse!("2019-W01", "{ISOweek}")
    # thursday
    assert ~N[2009-12-28 00:00:00] = Timex.parse!("2009-W53", "{ISOweek}")
    # wednesday
    assert ~N[2024-12-30 00:00:00] = Timex.parse!("2025-W01", "{ISOweek}")
    # friday
    assert ~N[2016-11-28 00:00:00] = Timex.parse!("2016-W48", "{ISOweek}")
    # saturday
    assert ~N[2022-01-03 00:00:00] = Timex.parse!("2022-W01", "{ISOweek}")
    # sunday
    assert ~N[2023-01-02 00:00:00] = Timex.parse!("2023-W01", "{ISOweek}")
  end
  ...
  test "parse full and abbreviated month names" do
    ...
    assert {:error, "Expected ` `, but found `J` at line 1, column 1."} =
             parse("January", " {Mshort}")
  end
  ...
  test "parse simple time formats" do
    ...
    assert {:error, "Expected `minute` at line 1, column 7."} =
             parse("pm 12:3:4", "{am} {h24}:{m}:{s}")
  end
  ...
  test "parse fractional seconds" do
    ...
    assert {:ok, %NaiveDateTime{second: 2, microsecond: {687_000, 3}}} =
             parse(str, "{YYYY}-{M}-{0D}T{h24}:{m}:{s}{ss}")
  end
  ...
  test "parse s-epoch" do
    ...
    assert {:error, "Expected `seconds since epoch` at line 1, column 1."} =
             parse("  0", "{s-epoch}")
  ...
  test "parse ASN1:GeneralizedTime" do
    ...
    timestamp = Duration.from_erl({1236, 295_519, 456_000}) |> Duration.to_seconds(truncate: true)
  ...
  test "parse ASN1:GeneralizedTime:Z" do
    ...
    assert 1_236_295_519 = Timex.to_unix(result)
  ...
  test "parse kitchen" do
    ...
    date =
      Timex.to_naive_datetime(
        Timex.set(Timex.now(), hour: 15, minute: 25, second: 0, microsecond: {0, 0})
      )
  ...
  test "parse ISO8601 (Extended)" do
    ...
    assert {:error, "Expected at least one digit" <> _} =
             parse("2014-08-14T12:34:33.Z", "{ISO:Extended}")
